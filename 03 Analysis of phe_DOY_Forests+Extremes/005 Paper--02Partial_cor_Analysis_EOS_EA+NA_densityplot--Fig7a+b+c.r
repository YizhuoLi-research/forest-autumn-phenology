...
##################  03 按气候-森林类型 取像元计算皮尔森（Pearson相关性）计算2--EOS/3--EOS个气候区间  ###########################
...
# 读取 phe_EOS_DOY 目录下的所有文件
phe_EOS_files <- list.files(phe_EOS_dir, full.names = TRUE)
...
# 筛选文件 # 10年phe4_6 extremes的 SpatRaster
TXx_phe4_6_files <- file_paths[grepl("^TXx_phe4_6", basename(file_paths))]
TNn_phe4_6_files <- file_paths[grepl("^TNn_phe4_6", basename(file_paths))]
DTR_phe4_6_files <- file_paths[grepl("^DTR_phe4_6", basename(file_paths))]
TXge30_phe4_6_files <- file_paths[grepl("^TXge30_phe4_6", basename(file_paths))]
TNlt2_phe4_6_files <- file_paths[grepl("^TNlt2_phe4_6", basename(file_paths))]
RainDay_phe4_6_files <- file_paths[grepl("^RainDay_phe4_6", basename(file_paths))]
CDD_phe4_6_files <- file_paths[grepl("^CDD_phe4_6", basename(file_paths))]
CWD_phe4_6_files <- file_paths[grepl("^CWD_phe4_6", basename(file_paths))]
Rx1day_phe4_6_files <- file_paths[grepl("^Rx1day_phe4_6", basename(file_paths))]
SDII_phe4_6_files <- file_paths[grepl("^SDII_phe4_6", basename(file_paths))]
merged_Duration_phe4_6_files <- file_paths[grepl("^merged_Duration_phe4_6", basename(file_paths))]
...
file_groups <- list( TXx_1_3 = TXx_phe1_3_files, TNn_1_3 = TNn_phe1_3_files,  DTR_1_3 = DTR_phe1_3_files, TXge30_1_3 = TXge30_phe1_3_files,
                     ...
                     TXx_4_6= TXx_phe4_6_files, TNn_4_6 = TNn_phe4_6_files,  DTR_4_6 = DTR_phe4_6_files, TXge30_4_6 = TXge30_phe4_6_files,
                     TNlt2_4_6 = TNlt2_phe4_6_files, RainDay_4_6 = RainDay_phe4_6_files,CDD_4_6 = CDD_phe4_6_files, CWD_4_6 = CWD_phe4_6_files,
                     Rx1day_4_6 = Rx1day_phe4_6_files, SDII_4_6 = SDII_phe4_6_files, merged_Duration_4_6 = merged_Duration_phe4_6_files)


analyze_correlation <- function(extremes_var, phe_EOS_DOY, classify_border, Climate_Class_list, forest_types, var_type, control_vars) {
  ...
  for (Climate_Class in Climate_Class_list) {
    ...
    phe_masked      <- mask(crop(phe_EOS_DOY, selected_border_val), selected_border_val)
    ...
    for (forest in forest_types) {
      ...
      colnames(df_val) <- c(paste0("extremes_var", 2013:2022),
                            paste0("phe_EOS_DOY", 2013:2022))
      
      # 提取数值
      extremes_val <- unlist(df_val[, grep("extremes_var", colnames(df_val), value = TRUE)])
      phe_val      <- unlist(df_val[, grep("phe_EOS_DOY", colnames(df_val), value = TRUE)])
      ...
      zero_var_cols <- names(variances[variances == 0])
        ...
        if (!is.na(zero_var_cols[1])) {  # 检查第一个元素是否为 NA
        cat("Removing control variables with zero variance:", zero_var_cols, "\n")
        new_df_val <- new_df_val[, !colnames(new_df_val) %in% zero_var_cols] }
...
perform_analysis <- function(file_groups) {
  ...
  for (var_type in names(file_groups)) {  
    ...
    # 读取 phe_EOS_DOY 数据
    phe_EOS_DOY <- rast(phe_EOS_files)
...
write.csv(results_df, "./0.figure/03 Paper/02Partial_Cor_33var_to_EOS.csv", row.names = FALSE)
...
results_df <- read.csv("./0.figure/03 Paper/02Partial_Cor_33var_to_EOS.csv", stringsAsFactors = FALSE)
...
result_df_4_6 <- results_df %>% filter(grepl("_4_6$", Variable))
...
result_df_4_6 <- result_df_4_6 %>% mutate(Variable = sub("_4_6$", "", Variable))
...
result_df_4_6$Variable <- gsub("RainDay", "Rainday", result_df_4_6$Variable)
...
result_df_4_6$Variable <- factor(result_df_4_6$Variable, levels = c("TXx", "TXge30", "TNn", "TNlt2", "DTR", "Rainday", "CWD", "CDD", "Rx1day", "SDII", "Duration"))
...
p1 <- ggplot(result_df_1_3, aes(x = Variable, y = Lab, fill = Partial_Correlation)) +
  ...
  geom_text(aes(label = Text), color = "black", size = 2.35,
...
print(p1)
ggsave(filename = "./0.figure/03 Paper/Fig.7.PartialCor_EOS_phe1_3(a).tiff",
       plot = p1,  width = 3.3,  height = 5,  units = "in",  dpi = 300)
...
print(p2)
ggsave(filename = "./0.figure/03 Paper/Fig.7.PartialCor_EOS_phe3_4(b).tiff",
       plot = p2,  width = 3.3,  height = 5,  units = "in",  dpi = 300)


######## 绘制相关性图2(GMO-GDO) #######


p3 <- ggplot(result_df_4_6, aes(x = Variable, y = Lab, fill = Partial_Correlation)) +
  geom_tile(color = "grey40") +                                 # 矩形块表示相关性，边框灰
  geom_text(aes(label = Text), color = "black", size = 2.35,
            vjust = 0.75, lineheight = 0.85) +  # vjust = 0.8更下, 控制垂直位置；调整 vjust 和行间距
  scale_fill_gradient2(
...
print(p3)
ggsave(filename = "./0.figure/03 Paper/Fig.7.PartialCor_EOS_phe4_6(c).tiff",
       plot = p3,  width = 4.2,  height = 5,  units = "in",  dpi = 300)
