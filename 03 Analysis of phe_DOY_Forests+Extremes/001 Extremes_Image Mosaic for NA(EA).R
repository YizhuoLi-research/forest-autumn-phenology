...
library(terra)
library(progress)
setwd("D:/Graduation_Thesis/01 Download/12 Tmin_download")
...
folder_path <- "./EA_Tmin/"   #NorthAmerica的数据
...
#识别文件夹年份并分类
file_years <- substr(basename(file_list), 16,19)   #Tmin:16-19;pre:18-21  prcp 18-21
...
# aaa <- rast("D:/Graduation_Thesis/01 Download/11 Tmax_download/EA_Tmax/2013/001.tif")
# summary(ccc)
# plot(ccc)
...
# rsrc <- sprc(band_rasters)
# merged_raster <- merge(rsrc, na.rm = TRUE)
# name        : 20130101_ETET_001 
# min value   :     -0.0001788710 
# max value   :      0.0001461269 
#算法2（NA）
