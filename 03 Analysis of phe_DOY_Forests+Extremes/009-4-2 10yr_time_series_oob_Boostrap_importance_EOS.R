...
phe_EOS_dir <- "./EA+NA_Results/merged_Phe_DOY_years_2_PHE_analysis/merged_phe4_DOY/"
  ...
  for (forest in forest_types) {
    ...
    tryCatch({
      ...
      df_base <- data.frame(
        ...
        EOS = as.vector(values(phe_masked_zone_forest)[valid_pixels, ])
      )
      ...
      for (y in 2013:2022) {
        ...
        df_scaled <- df_year %>% mutate(across(-EOS, safe_scale))
...
write.csv(final_importance_df, 
          file.path(output_dir, "EOS_10years_Indices_Importance_Results.csv"), 
          row.names = FALSE)
