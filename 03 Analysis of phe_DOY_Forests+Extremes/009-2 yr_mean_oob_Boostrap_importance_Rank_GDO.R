...
# # 确保所有栅格范围和分辨率一致
# # clim_rasters <- resample(clim_rasters, phe_GDO_rasters)
# # climate_zone <- resample(climate_zone, phe_GDO_rasters)
# # forest_type <- resample(forest_type, phe_GDO_rasters)
# 
# # 合并所有数据为一个多层栅格
# all_data <- c(phe_GDO_rasters, clim_rasters, climate_zone, forest_type)
# names(all_data) <- c(paste0("GDO_", 1:10), prefixes, "climate_zone", "forest_type")
# 
# # 转换为数据框（按像素提取值）
# df <- as.data.frame(all_data, xy = TRUE, na.rm = TRUE)
# 
# # 计算每个像素的GDO多年平均值（或其他统计量）
# df$GDO_mean <- rowMeans(df[, grep("GDO_", names(df))], na.rm = TRUE)
  ...
  for (forest in forest_types) {
    ...
    tryCatch({
      ...
      for (y in 2013:2022) {
        ...
        # 存储重要性
        imp <- importance(rf_year, type = 1, scale = FALSE)
        yearly_importance[[as.character(y)]] <- imp[, 1] # 第一列为%IncMSE
        valid_years <- c(valid_years, y)
      }
      
      # 合并结果
      if (length(yearly_importance) > 0) {
        ...
        result_df <- data.frame(
          ...
          IncMSE_mean = rowMeans(importance_matrix),
          IncMSE_SD = apply(importance_matrix, 1, sd),
          Years = length(valid_years),
          stringsAsFactors = FALSE
        )
        
        final_importance_df <- rbind(final_importance_df, result_df)
      }
...
write.csv(final_importance_df, 
          file.path(output_dir, "GDO_years_mean_Indices_Importance_Results.csv"), 
          row.names = FALSE)
