...
###### 1. 导入数据 ####
tmax <- rast(list.files("./01 Download/11 Tmax_download/NA_Tmax/2013/", pattern = ".tif$", full.names = TRUE))  # 1-365张图
tmin <- rast(list.files("./01 Download/12 Tmin_download/NA_Tmin/2013/", pattern = ".tif$", full.names = TRUE))  # 1-365张图
prcp <- rast(list.files("./01 Download/13 Precipitation_download/NA_Precipitation/2013/", pattern = ".tif$", full.names = TRUE))  # 1-365张图
PHE <- rast(c(
  "./NA_Results/2.phe_DOY_PHE_analysis/0common_pixel/phe1_DOY_2013.tif",
  "./NA_Results/2.phe_DOY_PHE_analysis/0common_pixel/phe2_DOY_2013.tif",
  "./NA_Results/2.phe_DOY_PHE_analysis/0common_pixel/phe3_DOY_2013.tif",
  "./NA_Results/2.phe_DOY_PHE_analysis/0common_pixel/phe4_DOY_2013.tif",
  "./NA_Results/2.phe_DOY_PHE_analysis/0common_pixel/phe5_DOY_2013.tif",
  "./NA_Results/2.phe_DOY_PHE_analysis/0common_pixel/phe6_DOY_2013.tif"
))
##### 缩小数据量-节省时间的试运行 ####
...
for (n in 1:ncell(tmax)) {
  ...
  if (is.na(tmax[[1]][n]) || is.na(tmin[[1]][n]) || is.na(prcp[[1]][n]) ||any(is.na(PHE[[1:6]][n]))) {
    next
  }
...
# 定义输出文件夹路径
output_dir <- "./NA_Results/3.Clim_Duration_Extremes/"
...
# 保存结果
# 1.温度指标 5层
writeRaster(Temp_phe1_3, "./NA_Results/3.Clim_Duration_Extremes/Temp_phe1_3_2013.tif", overwrite = TRUE)
writeRaster(Temp_phe3_4, "./NA_Results/3.Clim_Duration_Extremes/Temp_phe3_4_2013.tif", overwrite = TRUE)
writeRaster(Temp_phe4_6, "./NA_Results/3.Clim_Duration_Extremes/Temp_phe4_6_2013.tif", overwrite = TRUE)

# 2.降水指标 5层
writeRaster(Prcp_phe1_3, "./NA_Results/3.Clim_Duration_Extremes/Prcp_phe1_3_2013.tif", overwrite = TRUE)
writeRaster(Prcp_phe3_4, "./NA_Results/3.Clim_Duration_Extremes/Prcp_phe3_4_2013.tif", overwrite = TRUE)
writeRaster(Prcp_phe4_6, "./NA_Results/3.Clim_Duration_Extremes/Prcp_phe4_6_2013.tif", overwrite = TRUE)

# 3.物候指标 1层
writeRaster(Duration_phe1_3, "./NA_Results/3.Clim_Duration_Extremes/Duration_phe1_3_2013.tif", overwrite = TRUE)
writeRaster(Duration_phe3_4, "./NA_Results/3.Clim_Duration_Extremes/Duration_phe3_4_2013.tif", overwrite = TRUE)
writeRaster(Duration_phe4_6, "./NA_Results/3.Clim_Duration_Extremes/Duration_phe4_6_2013.tif", overwrite = TRUE)
