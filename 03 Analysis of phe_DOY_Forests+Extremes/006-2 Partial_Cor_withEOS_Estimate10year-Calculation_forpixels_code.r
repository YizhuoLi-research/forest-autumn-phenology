...
# 读取物候数据
phe_EOS_rasters <- rast(phe_EOS_files)
...
for (i in seq_along(prefixes)) {
  ...
  # 读取其他控制极端指标数据（32组，每组10个文件）
  control_files <- lapply(prefixes[-i], function(prefix) {
    files <- file_paths[grepl(paste0("^", prefix), basename(file_paths))]
  })
  control_files_rasters <- rast(unlist(control_files))  # 32个变量 * 10层
  
  # 对三套文件的共有像元进行处理
  sample1 <- phe_EOS_rasters
  ...
  phe_EOS_rasters <- phe_EOS_rasters * sample1[[1]] * sample2[[1]] * sample3[[1]]
  ...
  # 合并数据
  new_data <- c(phe_EOS_rasters, current_var_rasters, control_files_rasters)
  ...
  # 保存结果
  # 定义输出目录
  output_dir <- "./EA+NA_Results/merged_partial_corr_result/with_EOS"
  ...
  writeRaster(partial_corr_map, 
    filename = paste0("./EA+NA_Results/merged_partial_corr_result/with_EOS/partial_corr_", prefixes[i], ".tif"), 
     overwrite = TRUE)
}



raster1 <- rast("./EA+NA_Results/merged_partial_corr_result/with_EOS/partial_corr_TXx_phe1_3.tif")
raster2 <- rast("./EA+NA_Results/merged_partial_corr_result/with_EOS/partial_corr_TNn_phe1_3.tif")
